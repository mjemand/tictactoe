...
const checkColumnValues = () => {
    const values = matrix.map(cell =>
        cell.every((value) => value === 'X') ||
        cell.every((value) => value === 'O'))
    return values.indexOf(true) !== -1 ? true : false;
}
...
const checkWinner = () => {
    if (checkRowValues()) {console.log('Winner');}
    else if (checkColumnValues()) {console.log('Winner');}
